...
# .rds do not have clustering variables, so I revised other metadata, but this do not have clustering either
#metadata_tsv = "/lustre/scratch127/humgen/projects_v2/jaguar_analysis/analysis/fg14/Atlas_workflow_CITEseq/results_old/results/filtering/FREEZE_II_only_ATLAS08_to_14/cr_filtered_meta_FREEZE_II_only_ATLAS08_to_14.tsv.gz"
...
#query <- subset(query_all, subset = orig.ident == "ATLAS09a" & anydoublet == "singlet") # Example for testing

# --- Creare list of gene_sets ---
# create gene sets TCR and immunoglobuline genes
# TCR <- grep("^TRA|^TRB|^TRG|^TRD", rownames(query), value = TRUE)
# immunoglobuline_light_chain_V <- grep("^IGLV", rownames(query), value = TRUE) #variable regions
# immunoglobuline_light_chain <- c(immunoglobuline_light_chain_V,
#                                  grep("^IGLC|^IGLJ", rownames(query), value = TRUE))
# immunoglobuline_heavy_chain <- grep("^IGHV|^IGHD|^IGHJ|^IGHG|^IGHA|^IGHE|^IGHM", rownames(query), value = TRUE) #variable and joining regions
# 
...
# Only in this case, because we have few cells in sme populations
...
target_stages <- c("resting_CD", "early_activation_CD", "activated_CD")
...
plot_scores_overlap <- ggplot(df_overlap_viz, aes(x = Score_Type, y = Intensity, fill = Score_Type)) +
  ...
  theme_bw() +
  scale_fill_manual(values = c("resting_CD" = "#619CFF", 
                               "early_activation_CD" = "#0fa348", 
                               "activated_CD" = "#b82d14")) +
